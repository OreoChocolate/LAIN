...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, prior=None, query=None,mask = None, la_masks=None):
        ...
        x = torch.cat([query.unsqueeze(0), (self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype,
                      device=x.device)) + self.positional_embedding.to(x.dtype)[:1,:].unsqueeze(0), x + self.positional_embedding.to(x.dtype)[1:,:].unsqueeze(0)], dim=1)  # shape = [*, grid ** 2 + 1, width]
        ...
        return x[:,:-196,:], x[:,-196:,:].view(bs,h//self.patch_size, w//self.patch_size, -1).permute(0, 3, 1, 2)
